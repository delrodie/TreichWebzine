...
class SliderController extends Controller
{
    ...
    public function newAction(Request $request, Utilities $utilities)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $resume = $utilities->resume($slider->getContenu(), 139, '...', true);
            $slider->setContenu($resume);
            $em->persist($slider);
            $em->flush();
     ...
     * @Method({"GET", "POST"})
     */
    public function editAction(Request $request, Slider $slider, Utilities $utilities)
    {
        $deleteForm = $this->createDeleteForm($slider);
